...
(function(exports) {
  ...
  var updateSearch = function() {
    ...
    for(let i=0; items.length > i; i++) {
      ...
      if(itemText.toLowerCase().startsWith(currentSearchString.toLowerCase())) {
        numItemsFound += 1;
        if(selit === null) {
          selit = item;
          selectItem(selit);
  ...
  function loadDirectory(dir, affectHistory) {
    ...
    fs.readdir(dir, function(err, paths) {
      ...
      for(let i=0; paths.length > i; i++) {
        ...
        var locationPath = document.querySelector("fm-path");
        locationPath.innerHTML = "";
        var pathItems = dir.split(path.sep);
        pathItems.forEach(function(val, index) {
          let itemEl = document.createElement("fm-item");
          itemEl.textContent = val;
          locationPath.appendChild(itemEl);
        });
      }
    });
  }
